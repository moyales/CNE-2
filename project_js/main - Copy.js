...
google.charts.setOnLoadCallback(drawActualsChart);
google.charts.setOnLoadCallback(drawForecastChart);

function drawActualsChart() { 
...
function drawForecastChart() { 
    ...
    var options = {
        title: 'Expense Summary (Forecast)',
        is3D: false,
        piehole: 0.4
    };

    var chart = new google.visualization.PieChart(document.getElementById('forecastChartDiv'));
    
    chart.draw(data, options);
}

// Dependent Dropdown Stuff
...
/*$(document).on('change','#incomeTypeSelection',function(){
    ...
    if("oneTimeIncome" == selectedVal){
        var oneTimeList = '<select name="incomeSource"><option value="variableSideJob">Side Job</option><option value="variableSelling">Selling</option><option value="variableTrading">Trading</option><option value="variableOther">Other</option></select>';
        $('select').remove();
        $('#incomeSource').append(oneTimeList);
    }else if("recurringIncome" == selectedVal){
        var recurringList = '<select name="incomeSource"><option value="fixedAllowance">Allowance</option><option value="fixedWages">Wages</option>';
        $('select').remove();
        $('#incomeSource').append(recurringList);
    }
